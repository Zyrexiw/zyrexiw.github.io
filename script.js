...
function notify(msg) {
  ...
  function handleFormSubmit(formId, commentId, isEdit) {
    ...
    form.addEventListener('submit', e => {
      ...
      saveHistory({ action: isEdit?'modifié':'créé', index: isEdit?editIndex+1:getRapports().length, date: new Date() });
      notify(isEdit?'Rapport mis à jour !':'Rapport créé !');
      document.getElementById('edit-modal').classList.add('hidden');
      openReport(r); editIndex=null; renderRapportsList(); renderHistorique();
      form.reset(); document.getElementById(commentId).innerHTML='';
    });
  }
  ...
  function renderHistorique() {
    ...
    getHistory().forEach(e=> ul.innerHTML+=`<li><i class="fas ${e.action==='créé'?'fa-file-alt':e.action==='modifié'?'fa-edit':'fa-trash'}"></i> Rapport #${e.index} ${e.action} le ${new Date(e.date).toLocaleString()}</li>`);
  }
  function renderRapportsList() {
    ...
    getRapports().forEach((r,i)=>{
      ...
      ul.innerHTML+=`<li><span>#${i+1} - ${r.tutore.nom} (${date})</span><div>`+
  ...
  function openReport(r) {
    const w=window.open(); w.document.write(generateReportHTML(r)); w.document.close();
  }
  
  function openEditModal(i) {
    const r=getRapports()[i]; editIndex=i;
    const f=document.getElementById('edit-form');
    f['edit-report-date'].value=new Date(r.date).toISOString().slice(0,10);
    ['nom','rio','grade'].forEach(field=> f[`redacteur-${field}`].value=r.redacteur[field]);
    ['nom','rio','grade'].forEach(field=> f[`tutore-${field}`].value=r.tutore[field]);
    ['radio','conduite','adaptation'].forEach(field=> f[`critere-${field}`].value=r.evaluations[field]);
  ...
  function generateReportHTML(r) {
    ...
    return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Rapport</title>` +
      ...
      `<li>Adaptation: ${r.evaluations.adaptation||'Non évalué'}</li></ul>` +
  ...
  (function(){ renderRapportsList(); renderHistorique();
    const tb=document.getElementById('toggle-sidebar');
    tb.addEventListener('click',()=>document.querySelector('.sidebar').classList.toggle('open'));
    tb.addEventListener('dblclick',()=>document.querySelector('.sidebar').classList.toggle('collapsed'));
  })();