...
function handleFormSubmit(formId, commentaireId, isEdit) {
  ...
  form.addEventListener('submit', e => {
    ...
    const r = {
      date: new Date(),
      redacteur: {
        nom:   f['redacteur-nom'].value,
        rio:   f['redacteur-rio'].value,
        grade: f['redacteur-grade'].value
      },
      tutore: {
        nom:   f['tutore-nom'].value,
        rio:   f['tutore-rio'].value,
        grade: f['tutore-grade'].value
      },
      evaluations: {
        radio:      f['critere-radio'].value,
        conduite:   f['critere-conduite'].value,
        adaptation: f['critere-adaptation'].value
      },
...
function renderHistorique() {
  ...
  getHistory().forEach(entry => {
    ...
    const icon = entry.action === 'créé' ? 'fa-file-alt'
               : entry.action === 'modifié' ? 'fa-edit'
               : 'fa-trash';
    li.innerHTML = `<i class="fas ${icon}"></i><span>
      Rapport #${entry.index} ${entry.action} le ${new Date(entry.date).toLocaleString()}
    </span>`;
    ul.appendChild(li);
  });
}
function renderRapportsList() {
  ...
  getRapports().forEach((r, i) => {
    const li = document.createElement('li');
    li.innerHTML = `<span>#${i+1} - ${r.tutore.nom}</span>
      <div>
...
document.getElementById('cancel-edit').addEventListener('click', () => {
  document.getElementById('edit-modal').classList.add('hidden'); editIndex = null;
});
